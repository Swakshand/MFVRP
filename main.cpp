...
struct Vehicle{
    ...
    Vehicle(): capacity(0), available(0), tour(),ev(false), index(-1) {}
...
auto neighbor_relocate_ev(const vector<Vehicle>& vehicles, const vector<Customer>& customers, 
    ...
    for(auto & vehicle_a : vehicles){
        for(auto & vehicle_b : vehicles){
            ...
            for(auto node_index_a = 1; node_index_a < tour_a.size(); ++node_index_a){
                auto customer_index = tour_a[node_index_a].index;
                auto & customer = customers[customer_index];
                if(customer.demand > vehicle_b.available || customer.res) continue;
...
auto neighbor_exchange_ev(const vector<Vehicle> & vehicles, const vector<Customer> & customers, 
                const DistanceMatrix & distance_matrix, const Penalty & penalty, double lambda){
    auto max_augmented_cost_gain = -numeric_limits<double>::infinity();
    auto max_cost_gain = -numeric_limits<double>::infinity();
    auto max_vehicle_new_a = Vehicle();
        ...
        for(auto & vehicle_b : vehicles){
            ...
            for(auto node_index_a = 1; node_index_a < tour_a.size(); ++node_index_a){
                for(auto node_index_b = 1; node_index_b < tour_b.size(); ++node_index_b){
                    auto & customer_a = customers[tour_a[node_index_a].index];
                    auto & customer_b = customers[tour_b[node_index_b].index];
                    if(customer_a.res && !customer_b.res) continue;
                    if(!customer_a.res && customer_b.res) continue;
...
auto neighbor_two_opt_ev(const vector<Vehicle> & vehicles, const vector<Customer> & customers, 
                const DistanceMatrix & distance_matrix, const Penalty & penalty, double lambda){
    auto max_augmented_cost_gain = -numeric_limits<double>::infinity();
    auto max_cost_gain = -numeric_limits<double>::infinity();
...
auto neighbor_cross_ev(const vector<Vehicle> & vehicles, const vector<Customer> & customers, 
                const DistanceMatrix & distance_matrix, const Penalty & penalty, double lambda){
    auto max_augmented_cost_gain = -numeric_limits<double>::infinity();
    auto max_cost_gain = -numeric_limits<double>::infinity();
    auto max_vehicle_new_a = Vehicle();
...
auto neighbor_cross(const vector<Vehicle> & vehicles, const vector<Customer> & customers, const DistanceMatrix & distance_matrix,
            const Penalty & penalty, double lambda, map<Vehicle, bool>& is_vehicle_serving_residential){
    auto max_augmented_cost_gain = -numeric_limits<double>::infinity();
    auto max_cost_gain = -numeric_limits<double>::infinity();
    auto max_vehicle_new_a = Vehicle();
    auto max_vehicle_new_b = Vehicle();
    ...
    for(auto& vehicle_a : vehicles){
        for(auto& vehicle_b : vehicles){
            if(vehicle_a.index == vehicle_b.index) continue;
            if(!(is_vehicle_serving_residential[vehicle_a] ^ is_vehicle_serving_residential[vehicle_b])) continue;
...
auto search_ev(const vector<Customer> & customers, vector<Vehicle>& vehicles, map<Vehicle, bool>& is_vehicle_serving_residential){
    ...
    for(auto step = 0; step < 100; ++step){
        ...
        auto [relocate_augmented_cost_gain, relocate_cost_gain, relocate_vehicle_new_a, relocate_vehicle_new_b, relocate_feasible] = 
                neighbor_relocate_ev(vehicles, customers, distance_matrix, penalty, lambda);

        auto [exchange_augmented_cost_gain, exchange_cost_gain, exchange_vehicle_new_a, exchange_vehicle_new_b, exchange_feasible] = 
                neighbor_exchange_ev(vehicles, customers, distance_matrix, penalty, lambda);

        auto [two_opt_augmented_cost_gain, two_opt_cost_gain, two_opt_vehicle_new, two_opt_feasible] =
                neighbor_two_opt_ev(vehicles, customers, distance_matrix, penalty, lambda);
        ...
        auto [cross_augmented_cost_gain, cross_cost_gain, cross_vehicle_new_a, cross_vehicle_new_b, cross_feasible] =
                ...
                neighbor_cross_ev(vehicles, customers, distance_matrix, penalty, lambda);
        ...
        if(best_cost > cost){
            ...
            save_result("cpp_output_ev.txt", best_cost, best_vehicles);
        }
    }

    for(auto i : best_vehicles){
        for(int j=1; j<i.tour.size()-1; j++){
            int customer_index = i.tour[j].index;
            if(customers[customer_index - 1].res){
                is_vehicle_serving_residential[i] = true;
                break;
            }
        }
    }
...
// auto search(const vector<Customer> & customers, vector<Vehicle> & vehicles, map<Vehicle, bool>& is_vehicle_serving_residential,
//             vector<Customer>& customer_non_res, vector<Vehicle>& remaining_vehicles){
auto search(vector<Customer> & customers, vector<Vehicle> & vehicles, map<Vehicle, bool>& is_vehicle_serving_residential,
            vector<Customer>& customer_non_res, vector<Vehicle>& remaining_vehicles){
    // auto distance_matrix = init_distance_matrix(customers);
    auto distance_matrix = init_distance_matrix(customer_non_res);
    auto penalty = Penalty(distance_matrix.size(), vector<int>(distance_matrix.size(), 0));

    // init_tour(customers, vehicles, distance_matrix);
    init_tour(customer_non_res, remaining_vehicles, distance_matrix);





    customers = customer_non_res;
    ...
    for(auto step = 0; step < 100; ++step){
        ...
        if(best_cost > cost){
            best_cost = cost;
            best_vehicles = vehicles;
            save_result("cpp_output_total.txt", best_cost, best_vehicles);
        }
    }
...
int main() {
    ...
    cout << "Enter n_customer_cv, n_customer_ev, n_vehicle_cv, n_vehicle_ev, capacity_ev, capacity_cv: "<<endl;
    cin >> n_customer_cv>>n_customer_ev >> n_vehicle_cv>>n_vehicle_ev >> capacity_ev>>capacity_cv;
    ...
    for (auto &customer : customers) {
        ...
        bool d;
        cin >> a >> b >> c>> d;
        customer.demand = a, customer.x = b, customer.y = c, customer.res = d;
        if(d){
    ...
    for (int i = n_vehicle_cv; i < n_vehicle_ev + n_vehicle_cv; i++) {
        Vehicle vehicle;
    ...
    map<Vehicle, bool> is_vehicle_serving_residential;

    search_ev(customers_ev, vehicles_ev, is_vehicle_serving_residential);
    for(auto i : vehicles_ev){
        if(is_vehicle_serving_residential[i] == false) remaining_vehicles.push_back(i);
    }
    vector<Customer> customer_non_res;
    for(auto i : customers){
        if(i.res == false) customer_non_res.push_back(i);
    }
    search(customers, vehicles, is_vehicle_serving_residential, customer_non_res, remaining_vehicles);
    for(auto i : is_vehicle_serving_residential) cout<<i.first.index<<" "<<i.second<<endl;
    return 0;
}